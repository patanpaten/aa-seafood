...
@section('title', 'Ubah Tempat Beli')
        ...
        <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
            <div class="p-8 sm:p-12">
                <div class="flex items-center space-x-4 mb-10">
                    ...
                    <div>
                        <h1 class="text-2xl font-extrabold text-slate-900 uppercase tracking-tight">Ubah Tempat Beli</h1>
                        <p class="text-slate-500 text-sm font-medium">Perbarui data tempat Anda membeli barang.</p>
                    </div>
                </div>
                ...
                <form action="{{ route('suppliers.update', $supplier) }}" method="POST" class="space-y-8">
                    ...
                    <div class="space-y-6">
                        <div>
                            <label for="name" class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 ml-1">Nama Tempat Beli</label>
                            <input type="text" name="name" id="name" value="{{ $supplier->name }}" placeholder="Masukkan nama tempat beli"
                        ...
                        <div>
                            ...
                            <input type="text" name="contact" id="contact" value="{{ $supplier->contact }}" placeholder="Nomor telepon atau WA"
                        ...
                        <div>
                            ...
                            <textarea name="address" id="address" rows="3" placeholder="Alamat lengkap tempat beli"
                                class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition font-semibold text-slate-700">{{ $supplier->address }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="pt-4 flex flex-col sm:flex-row gap-4">
                        ...
                        <a href="{{ route('incoming-stocks.create') }}" class="flex-1 inline-flex items-center justify-center bg-slate-100 hover:bg-slate-200 text-slate-500 font-extrabold py-5 rounded-2xl transition-all transform active:scale-[0.98] uppercase tracking-widest text-sm">
                            Batal
                        </a>
                    </div>
                </form>
