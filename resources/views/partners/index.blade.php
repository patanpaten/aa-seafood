...
@section('title', 'Daftar Pelanggan Resto')

@section('content')
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight uppercase">Daftar Pelanggan Resto</h1>
            <p class="text-slate-500 font-medium text-sm mt-1">Daftar pelanggan restoran yang membeli barang Anda.</p>
        </div>
        <a href="{{ route('partners.create') }}" class="inline-flex items-center justify-center px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-2xl shadow-lg shadow-blue-200 transition-all transform active:scale-95 uppercase text-xs tracking-widest">
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors group">
                        <td class="px-8 py-6">
                            <span class="font-bold text-slate-700 group-hover:text-blue-600 transition-colors">{{ $partner->name }}</span>
                        </td>
                        <td class="px-8 py-6">
                            <span class="text-sm font-medium text-slate-500">{{ $partner->contact ?? '-' }}</span>
                        </td>
                        <td class="px-8 py-6">
                            <span class="text-sm font-medium text-slate-500">{{ $partner->address ? str($partner->address)->limit(40) : '-' }}</span>
                        </td>
                        <td class="px-8 py-6 text-center">
                            <div class="flex items-center justify-center space-x-3">
                                <a href="{{ route('partners.edit', $partner) }}" class="p-2 text-blue-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all">
                    ...
                    <tr>
                        <td colspan="4" class="px-8 py-12 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-slate-400 font-medium italic">Belum ada data pelanggan.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
